...
import {resendVerifyMail} from '../../api/auth.api';
...
const ResendVerifyMail: React.FC = ({}) => {
  const [loading, setLoading] = useState<boolean>(false);
  const navigation = useNavigation();

  return (
    <FormContainer>
      <HeadingText>Didn't receive verification e-mail?</HeadingText>
      <SubtitleText>
        Enter your email and we will send you a new verification e-mail.
      </SubtitleText>
      <Formik<{
        email: string;
      }>
        ...
        onSubmit={async (values, {setErrors, resetForm}) => {
          ...
          try {
            await resendVerifyMail(values.email);
            ToastAndroid.show('New e-mail has been sent.', ToastAndroid.LONG);
            resetForm();
